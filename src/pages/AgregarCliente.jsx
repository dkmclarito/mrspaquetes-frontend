...
const AgregarCliente = () => {
  ...
  useEffect(() => {
    const fetchTiposPersonas = async () => {
      try {
        ...
        if (
          response.data.tipo_persona &&
          Array.isArray(response.data.tipo_persona)
        ) {
          setTiposPersonas(response.data.tipo_persona);
        } else {
          console.error(
            "Respuesta no válida para tipos de personas:",
            response.data
          );
        }
      } catch (error) {
    ...
    const fetchDepartamentos = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          console.error(
            "Respuesta no válida para departamentos:",
            response.data
          );
        }
      } catch (error) {
      ...
      if (departamento) {
        try {
          ...
          if (
            ...
            Array.isArray(response.data.municipio)
          ) {
  ...
  const handleSearchGiro = (e) => {
    ...
    if (searchTerm.length > 0) {
      const filtered = giros.filter((g) => {
        ...
        const descripcion = g.st_descripcion
          ? g.st_descripcion.toLowerCase()
          : "";
  ...
  const handleEmailChange = (e) => {
    ...
    const emailPattern =
      /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(com|org|net|edu|gov|mil|info|biz|co|us|ca)$/;
  ...
  const handleTelefonoChange = (e) => {
    ...
    if (
      telefonoValue.length > 0 &&
      !["6", "7", "2"].includes(telefonoValue[0])
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    const clienteData = {
      ...
      nombre_empresa: tipoPersona === "2" ? nombreEmpresa : null,
      nit: tipoPersona === "2" ? nit : null,
      nrc: tipoPersona === "2" ? nrc : null,
      giro: giroValue,
    ...
    try {
      const response = await axios.post(
        `${API_URL}/admin-registrar-cliente`,
        clienteData,
        {
          headers: {
    ...
    if (error.response && error.response.data) {
      ...
      if (errorData) {
        ...
        if (Array.isArray(errorData)) {
          if (
            errorData.includes("El DUI ya está registrado.") &&
            errorData.includes("El teléfono ya está registrado.")
          ) {
            messages.push("El DUI y el teléfono ya están registrados.");
          } else if (
            errorData.includes("El NIT ya está registrado.") &&
            errorData.includes("El teléfono ya está registrado.")
          ) {
        ...
        if (typeof errorData === "object") {
          ...
          if (errorKeys.includes("dui") && errorKeys.includes("telefono")) {
            ...
            errorKeys.includes("nit") &&
            errorKeys.includes("telefono")
          ) {
            messages.push("El NIT y el teléfono ya están registrados.");
          } else if (errorKeys.includes("dui") && errorKeys.includes("email")) {
            messages.push(
              "El DUI y el correo electrónico ya están registrados."
            );
                        ...
                        <FormGroup className="form-group-custom">
                          ...
                          {!isDuiValid && (
                            <FormFeedback className="text-danger">
                              El DUI ingresado no es válido. Debe tener el
                              formato 02345678-9.
                            </FormFeedback>
                          )}
                        </FormGroup>
                    ...
                    <Row form>
                      ...
                      <Col md={6}>
                        <FormGroup className="form-group-custom">
                          ...
                          >
                            <option value="">Seleccione</option>
                            {municipiosPorDepartamento[departamento]?.map(
                              (mun) => (
