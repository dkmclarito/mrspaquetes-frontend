...
const ModalEditarVehiculo = ({
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        ...
        const [marcasRes, modelosRes, empleadosRes, estadosRes] =
          await Promise.all([
            axios.get(`${API_URL}/dropdown/get_marcas`, {
              headers: { Authorization: `Bearer ${token}` },
            }),
            axios.get(`${API_URL}/dropdown/get_modelos`, {
              headers: { Authorization: `Bearer ${token}` },
            }),
            axios.get(`${API_URL}/dropdown/get_empleados`, {
              headers: { Authorization: `Bearer ${token}` },
            }),
            axios.get(`${API_URL}/dropdown/get_estado_vehiculos`, {
              headers: { Authorization: `Bearer ${token}` },
            }),
          ]);
      ...
      const fetchVehicleData = async () => {
        try {
          ...
          const { data } = await axios.get(
            `${API_URL}/vehiculo/${vehiculoEditado.id}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
          ...
          const conductorEmpleado = empleados.find(
            (empleado) =>
              `${empleado.nombres} ${empleado.apellidos}` ===
              data.conductor?.trim()
          );
          const apoyoEmpleado = empleados.find(
            (empleado) =>
              `${empleado.nombres} ${empleado.apellidos}` === data.apoyo?.trim()
          );
          ...
          setVehiculoEditado((prev) => ({
            ...
            ...data,
            id_marca:
              marcas.find((marca) => marca.nombre === data.marca)?.id || "",
            id_modelo:
              modelos.find((modelo) => modelo.nombre === data.modelo)?.id || "",
            id_empleado_conductor: conductorEmpleado
              ? conductorEmpleado.id
              : "",
            id_empleado_apoyo: apoyoEmpleado ? apoyoEmpleado.id : "",
            id_estado:
              estados.find((estado) => estado.estado === data.estado)?.id || "",
  ...
  const handleInputChange = (event) => {
    ...
    const sanitizedValue =
      name === "capacidad_carga" ? value.replace(/[^0-9.]/g, "") : value;
  ...
  const isMoto = vehiculoEditado?.tipo === "moto";
